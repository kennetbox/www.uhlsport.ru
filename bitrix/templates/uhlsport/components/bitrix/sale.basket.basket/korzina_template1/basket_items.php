...
<? if($_REQUEST["update"] == 1):?>
	...
	<div id="bx-soa-main-notifications">
		...
		<div data-type="informer" class="alert alert-warning">
			<div class="row">
				<div class="col-xs-12" style="position: relative; padding-left: 48px;">
					<div class="icon-warning"></div>
					<div>
						При добавлении в Корзину товаров "на заказ со склада в Германии",
						<br />
						в Корзине остаются товары только этого типа для формирования отдельного заказа.
						<br />
						Продолжить оформление заказа Вы можете нажав кнопку "Оформить заказ".
					</div>
				</div>
			</div>
		</div>
...
<?
	...
	$removeElms = array();
	$stockInGermanyElm = array();
	foreach ($arResult["GRID"]["ROWS"] as $k => $arItem) {
		// $res = CIBlockElement::GetByID(2465);
		...
		if(!in_array($stockInGermany, $groupIDArray)) {
			$removeElms[$ELEMENT_ID] = $ID;
		} else {
			$stockInGermanyElm[$ELEMENT_ID] = $ID;
		}
	}
	// Удаляем "Товары" из "Корзины", если в Корзине присутствуют товары из раздела "Склад в Германии"
	if(count($removeElms) > 0 && count($stockInGermanyElm) > 0 && CModule::IncludeModule("sale")) {
		foreach($removeElms as $key => $val) {
			CSaleBasket::Delete($val);
		}
		LocalRedirect("index.php?update=1"); // Перезагрузка "Корзины", если в нее помещены товары
	}
?>
...
<div id="basket_items_list">
					...
					<tr id="<?=$arItem["ID"]?>">
						...
						?>
							<td class="control">
								...
								if ($bDelayColumn):
								?>
									<a style="display: none;" href="<?=str_replace("#ID#", $arItem["ID"], $arUrls["delay"])?>"><?=GetMessage("SALE_DELAY")?></a>
								<?
								endif;
								?>
							</td>
	...
	<div class="bx_ordercart_order_pay">
		...
		<div class="bx_ordercart_order_pay_left" style="display:none;">
		...
		<div class="bx_ordercart_order_pay_right" style="float: right;">
			<table class="bx_ordercart_order_sum">
				...
				<?if ($arParams["PRICE_VAT_SHOW_VALUE"] == "Y"):?>
					<tr>
						...
						<td id="allSum_wVAT_FORMATED" style="white-space: nowrap;"><?=$arResult["allSum_wVAT_FORMATED"]?></td>
					</tr>
					
					<? /* [Скрываем данные об НДС]
					<tr>
