	...
	BX.Sale.OrderAjaxComponent = {
		...
		locationsCompletion: function()
		{
			...
			for (i in this.locations)
			{
				...
				if (clearButton)
				{
					BX.bind(clearButton, 'click', BX.proxy(function(e){
						var target = e.target || e.srcElement,
						...
						if (this.validation.properties && this.validation.properties[i]
							&& (this.validation.properties[i].IS_LOCATION == 'Y' || this.validation.properties[i].IS_LOCATION4TAX == 'Y')
						)
							this.sendRequest();
					}, this));
				}
		...
		showActualBlock: function()
		{
			...
			while (allSections[i])
			{
				...
				if (!this.checkBlockErrors(allSections[i]) || !this.checkPreload(allSections[i]) || !allSections[i + 1])
				{
		...
		changeVisibleContent: function()
		{
			...
			{
				state = !!this.result.IS_AUTHORIZED
					|| sections[i].getAttribute('data-visited') == 'true'
					|| sections[i + 1] && sections[i + 1].getAttribute('data-visited') == 'true';
