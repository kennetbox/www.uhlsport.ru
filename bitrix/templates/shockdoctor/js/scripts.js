...
Enterprise.TopCart= {
    initialize: function (container, linkc, removeUrl) {
    	this.containerName = container;
        //this.container = $(container);
        this.removeURL = removeUrl;
        this.element = $(this.containerName).up(0);
        this.elementHeader = $(linkc);
        this.intervalDuration = 4000;
        this.interval = null;
        this.intervalToShowDuration = 400;
        this.intervalToShow = null;
        this.onElementMouseOut = this.handleMouseOut.bindAsEventListener(this);
        this.onElementMouseOver = this.handleMouseOver.bindAsEventListener(this);

        this.onElementHeaderMouseOut = this.handleHeaderMouseOut.bindAsEventListener(this);
        this.onElementHeaderMouseOver = this.handleHeaderMouseOver.bindAsEventListener(this)
        ...
        this.elementHeader.observe('mouseout', this.onElementHeaderMouseOut);
        this.elementHeader.observe('mouseover', this.onElementHeaderMouseOver);

        this.elementHeader.topCartCls = this;

    },

    handleMouseOut: function (evt) {
        if($(this.elementHeader).hasClassName('expanded') || $($(this.containerName).id).hasClassName('process') ) {
    ...
    handleHeaderMouseOut: function (evt) {
        if (this.intervalToShow !== null) {
            clearTimeout(this.intervalToShow);
            this.intervalToShow = null;
        }
    ...
    handleHeaderMouseOver: function (evt) {
    	if (!$(this.elementHeader).hasClassName('expanded') && !$($(this.containerName).id).hasClassName('process') && this.intervalToShow === null)  {
    		this.intervalToShow = setTimeout(this.showCart.bind(this), this.intervalToShowDuration);
    	}
    ...
    showCart: function (timePeriod) {
    	...
    	if (!$(this.elementHeader).hasClassName('expanded') && !$($(this.containerName).id).hasClassName('process') )  {
	        $(this.containerName).parentNode.style.zIndex=992;
            if ( document.viewport.getScrollOffsets()[1] > $('cartHeader').cumulativeOffset()[1] + $('cartHeader').getHeight() ) {
                document.body.id = 'body';
                new Effect.ScrollTo('body', { duration:'0.2' });
            }
	        new Effect.SlideDown($(this.containerName).id, { duration: 0.5,
	            beforeStart: function(effect) {$( effect.element.id ).addClassName('process');},
	            afterFinish: function(effect) {$( effect.element.id ).removeClassName('process'); }
	            });
	        $(this.elementHeader).addClassName('expanded');
	        if(typeof(timePeriod) !== 'undefined' && timePeriod) {
	            this.timePeriod = timePeriod*1000;
	            this.interval = setTimeout(this.hideCart.bind(Enterprise.TopCart), this.timePeriod);
	        }
    	}
    },

    showCartTimed: function() {
    	this.showCart(7);
    },
    ...
    hideCart: function () {
        ...
        if (!$($(this.containerName).id).hasClassName('process') && $(this.elementHeader).hasClassName('expanded')) {
            new Effect.SlideUp($(this.containerName).id, { duration: 0.5,
                beforeStart: function(effect) {$( effect.element.id ).addClassName('process');},
    ...
    removeItem: function(id, element) {
    	...
    	$(element).up().up().style.cursor = 'wait';
    	...
    	new Ajax.Request(
    			this.removeURL,
				{
					...
					onSuccess: function(response) {
						if (response.responseJSON['cart_block'] && response.responseJSON['cart_block'] != '') {
							$('topCartContent').update(response.responseJSON['cart_block']);
							$$('#cartHeader span')[0].update(response.responseJSON['cart_items_count']);
							//var fx = function() {$(this.containerName)}.bind(Enterprise.TopCart);
						}
					},
...
jQuery(document).ready(function() {
    jQuery(".featured-products-group > div").slick({
        autoplay: false,
        autoplaySpeed: 5000,
        dots: false,
        arrows: true,
