...
<?
global $SPORTS_NAME, $filteredColorName; // Название текущего вида спорта
?>
    ...
    <div class="list-header">
        <div class="list-header-contents">
            <div class="category-list-title">
                <h1>Показать все <?= $SPORTS_NAME ?>:</h1>
        ...
        <div class="list-products">
            <div class="list-products category-products">
                ...
                if (!empty($arResult['ITEMS'])) {
                    ...
                    <?
// Если собираем со всех разделов, то сортируем по цене "на ходу"
                    if ($arParams["SECTION_ID"] == 0 && (isset($_REQUEST['sort']) && $_REQUEST['sort'] == 'BASE')) {
                        ...
                        foreach ($arResult['ITEMS'] as $key => $arItem) {
                            ...
                            if (!empty($arItem['MIN_PRICE'])) {
                                ...
                                if ('N' == $arParams['PRODUCT_DISPLAY_MODE'] && isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
                                    ...
                                    $arResult['ITEMS'][$key]["MIN_PRICE_VALUE"] = $arItem['MIN_PRICE']['DISCOUNT_VALUE'];
                                ...
                                if ('Y' == $arParams['SHOW_OLD_PRICE'] && $arItem['MIN_PRICE']['DISCOUNT_VALUE'] < $arItem['MIN_PRICE']['VALUE']) {
                                    ...
                                    $arResult['ITEMS'][$key]["MIN_PRICE_VALUE"] = $arItem['MIN_PRICE']['VALUE'];
                                }
                            }
                        }

                        if (isset($_REQUEST['order']) && $_REQUEST['order'] == 'ASC') {
                            // Сортируем массив по цене, когда выводим со всех разделов
                            usort($arResult['ITEMS'], function($a, $b) {
                                if ($a['MIN_PRICE_VALUE'] == $b['MIN_PRICE_VALUE'])
                                    return 0;
                                return $a['MIN_PRICE_VALUE'] > $b['MIN_PRICE_VALUE'] ? 1 : -1;
                            });
                        }
                            ...
                            <a class="bx_catalog_item_images" href="<? echo $elementHref; //$arItem['DETAIL_PAGE_URL'];         ?>" alt="<? echo $imgTitle; ?>">
                                ...
                                <div class="<? echo ($arItem['SECOND_PICT'] ? 'bx_catalog_item double' : 'bx_catalog_item'); ?>">
                                    <div class="bx_catalog_item_container" id="<? echo $strMainID; ?>">
                                        ...
                                        $file = CFile::ResizeImageGet($arFile, array('width' => 190, 'height' => 145), BX_RESIZE_IMAGE_EXACT, true);
                                        $img = '<img id="' . $arItemIDs['PICT'] . '" src="' . $file['src'] . '" width="' . $file['width'] . '" height="' . $file['height'] . '" />';
                                        echo $img;
                                        ?>

                                        <? /*
                                          <img height="145" id="<? echo $arItemIDs['PICT']; ?>" src="<? echo $arItem['PREVIEW_PICTURE']['SRC']; ?>" alt="" />
                                         */ ?>
        ...
        <div class="list-toolbar">
            ...
            foreach ($arResult['ITEMS'] as $key => $val) {
                ...
                if ($ar_res = $res->GetNext()) {
                    $sectionsArr[$val['IBLOCK_SECTION_ID']] = $ar_res['NAME'];
                }
            }
            $GLOBALS["SECTIONS_ARR"] = $sectionsArr;
            ?>

            <?
            // Фильтр по свойству "Вид спорта"
            if (isset($_REQUEST["SPORTS"]) && !empty($_REQUEST["SPORTS"]) && is_string($_REQUEST["SPORTS"])) {
                $arFilter = Array('CODE' => $_REQUEST["SPORTS"], 'GLOBAL_ACTIVE' => 'Y');
                $db_list = CIBlockSection::GetList(Array(), $arFilter, true);
                if ($ar_result = $db_list->GetNext()) {
                    $SPORTS_ID = $ar_result['ID'];
                    $SPORTS_NAME = $ar_result['NAME'];
                }
            } else {
                $SPORTS_ID = 0; // Выводим все элементы
                $SPORTS_NAME = '';
            }
            $arrFilter = array(
                //"ARTNUMBER" => 555,
                "PROPERTY_SPORT_TYPE" => Array($SPORTS_ID),
            );

            $GLOBALS["SPORTS_NAME"] = $SPORTS_NAME;

            $CURRENT_SECTION_ID = ( isset($_REQUEST['section_id']) && !empty($_REQUEST['section_id']) ) ? $_REQUEST['section_id'] : 0;

//            $APPLICATION->IncludeComponent(
//                    "bitrix:catalog.smart.filter", "visual_vertical_stype_shockdoctor", Array(
//                "IBLOCK_TYPE" => "catalog", //$arParams["IBLOCK_TYPE"],
//                "IBLOCK_ID" => 16,
//                "SECTION_ID" => $CURRENT_SECTION_ID, //$arCurSection['ID'],
