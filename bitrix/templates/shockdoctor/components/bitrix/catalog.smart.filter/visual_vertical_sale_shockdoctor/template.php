...
<?
		global $SPORTS_NAME, $SECTIONS_ARR;
...
<div class="bx_filter_vertical bx_<?=$arParams["TEMPLATE_THEME"]?>">
	...
	<div class="bx_filter_section m4">
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			<? /* [Фильтр по категориям отключен]
			<dt>
				Категории<? if( isset($_REQUEST['section_id']) && !empty($_REQUEST['section_id']) ) { ?>: <?=$SECTIONS_ARR[$_REQUEST['section_id']]?><? } ?>
			</dt>
			<dd>
				<?
					$requestUri = $_SERVER['REQUEST_URI'];
				...
				<? if( isset($_REQUEST['section_id']) && !empty($_REQUEST['section_id']) ) { ?>
					...
					<div class="bx_filter_block">
						<? $printSortUrl[] = $requestUriArr[0]."?".$printSortUri.$amp; ?>
						<a href="<?=$printSortUrl[0]?>" class="btn-remove">Сбросить категорию</a>
					</div>
				...
				<? } else { ?>
					...
					<?
						foreach($SECTIONS_ARR as $key => $val) {
						$printSortUrl[$key] = $requestUriArr[0]."?".$printSortUri.$amp."section_id=".$key;
					?>
						<li>
							<a href="<?=$printSortUrl[$key] ?>"><?=$val?></a>
						</li>
					<?	
						}
					?>
			...
			*/?>
					...
					*/?>
					...
					<? if($arItem["CODE"] == 'SPORT_TYPE') { ?>
						...
						?>
								...
								<?
									if($ar["VALUE"] == $SPORTS_NAME) {
											$sportTypeFilter = "&".$ar["CONTROL_NAME"]."=Y";
									}
								?>
						...
						<? /* Здесь не публикуем фильтр, он не понадобится при просмотре по виду спорта
						<dt>
					...
					<? } elseif($arItem["CODE"] == 'COLOR_REF') { ?>

					<?
						$isFilteredColor = false;
						$filteredColorName = '';
						$currentColorsList = array();
						foreach($arItem["VALUES"] as $val => $ar):
					?>
							<?

									$currentColorsList[] = $val;
									...
									foreach($uriArr as $key => $val) {
											 ...
											 if($val[0] == 'COLORREF') continue;
											
											// Сброс всех фильтров
											if(
													false === strpos($val[0], $ar["FILTER_NAME_ALL"]) && 
													false === strpos($val[0], "section_id")
												) {
													if(!empty($val[0]) && $val[0] != '') {
															$priceSortUriAll[$key] = $val[0]."=".$val[1];
													}
											...
											if(
													...
													$filteredColorName = $ar["VALUE"];
													$GLOBALS["filteredColorName"] = $filteredColorName;
											}
									}
									$printSortUri = (count($priceSortUri) > 0)? implode('&', $priceSortUri) : '';
									$printSortUriAll = (count($priceSortUriAll) > 0)? implode('&', $priceSortUriAll) : '';
									$amp = ($printSortUri == '')? "" : "&" ;
									$printClearUrl = $requestUriArr[0]."?".$printSortUri.$sportTypeFilter; // URL без текущего фильтра
									
									$printClearUrlAll = $requestUriArr[0]."?".$printSortUriAll;
									
									$amp = $amp.$ar["CONTROL_NAME"];
									$printSortUrl = $requestUriArr[0]."?".$printSortUri.$amp."=Y".$sportTypeFilter."&COLORREF=";
							?>
							...
							<div class="bx_filter_block">
										...
										?>
												<li>
													...
													<a href="<?=$ar["DATA"]["printSortUrl"].$currentColorsList[$count]?>" title="<?=$ar["DATA"]["NAME"]?>"><?/* [title="<?=$ar["FILTER_NAME"]?>"] */?>
			...
			<? if($isFilteredColor || isset($_REQUEST['section_id']) && !empty($_REQUEST['section_id']) ) { ?>
					<p class="currently">
